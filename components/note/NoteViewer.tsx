...
export default function NoteViewer({ fileUrl, viewerUser, isLocked, onUnlock, isUnlocking, price = 1, errorMessage }: NoteViewerProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-muted/20 relative">
            ...
            {isLocked && !isLoading && (
                <div className="absolute inset-0 z-[60] backdrop-blur-sm bg-background/40 flex items-center justify-center pointer-events-auto">
            ...
            {!isLocked && (
                <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-card/90 backdrop-blur-md px-6 py-3 rounded-full border border-primary/30 shadow-2xl transition-transform hover:scale-105 select-none">
                    <button
                        onClick={zoomOut}
                        className="p-1 text-foreground hover:text-primary transition-colors active:scale-90"
                        title="Uzaklaştır"
                    >
                        <ZoomOut className="w-6 h-6" />
                    </button>
                    <div className="w-px h-6 bg-border"></div>
                    <span className="text-sm font-bold text-primary min-w-[3rem] text-center select-none font-mono">
            ...
            <div className={`flex-1 w-full overflow-y-auto bg-muted/20 scroll-smooth ${isLocked ? 'overflow-hidden pointer-events-none' : ''}`}>
                <div className="max-w-max mx-auto px-4 py-20 min-h-full flex flex-col items-center gap-8">
                    ...
                    <Document
                        ...
                        error={
                            <div className="mt-20 p-6 bg-destructive/10 border border-destructive/50 rounded-xl text-center">
                                <p className="text-destructive font-bold mb-2">PDF görüntülenemedi</p>
                                <p className="text-xs text-destructive/70">Dosya bozuk olabilir veya tarayıcı desteği yok.</p>
                            </div>
                        }
                    >
                        {Array.from(new Array(numPages), (el, index) => {
                            ...
                            return (
                                <div key={`page_${index + 1}`} className="relative group shadow-lg transition-transform duration-200">
                                    ...
                                    {isLocked && index === 0 && (
                                        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-background/60 to-background z-20"></div>
                                    )}
                                </div>
                            );
                        })}
